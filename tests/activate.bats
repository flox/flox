...
setup_file() {
  ...
  "$FLOX_BIN" --bash-passthru install -e "$TEST_ENVIRONMENT" hello cowsay;

  # Run by various shells to test that `flox activate ... -- ...;' works.
  _inline_cmd="$FLOX_BIN --bash-passthru activate -e '$TEST_ENVIRONMENT'";
  _inline_cmd="$_inline_cmd -- sh -c 'hello|cowsay'";
  export _inline_cmd;

  # Run by various shells to test that `eval "$( flox activate ...; )";' works.
  _eval_cmd="eval \"\$( $FLOX_BIN --bash-passthru activate -e '$TEST_ENVIRONMENT'; )\"";
...
@test "'flox activate' accepts '-s,--system' options" {
  run "$FLOX_BIN" --bash-passthru activate -e "$TEST_ENVIRONMENT" --system "$NIX_SYSTEM"  \
                           -- sh -c ':'
  assert_success
  run "$FLOX_BIN" --bash-passthru activate -e "$TEST_ENVIRONMENT" -s "$NIX_SYSTEM"  \
                           -- sh -c ':'
  assert_success
}
