...
def test_python_integration_with_flox(
    ...
    res = run(f"{flox} init", cwd=flox_env.path)
    ...
    with spawn(f"{flox} activate", cwd=flox_env.path) as shell:
        shell.expect(r"Building environment...", timeout=10)
        shell.expect_prompt(timeout=90)

        assert (flox_env.run_path / "bin/python3").exists()
        assert (flox_env.run_path / "bin/pip").exists()

        with open(flox_env.path / ".flox/pip.ini") as f:
