...
let
  ...
  # rustc 1.90 links x86_64-linux binaries with its own rust-lld, which is not
  # reached through the nixpkgs ld wrapper and so writes no RUNPATH. The build
  # opts out of the self-contained linker below, which means the external one
  # has to be supplied here, as `pkgs/rust-external-deps` does for crane.
  # Only that target: elsewhere the `-linker` value is still unstable.
  # <https://github.com/rust-lang/rust/issues/162781>
  linkerInputs = lib.optionals (stdenv.hostPlatform.system == "x86_64-linux") [
