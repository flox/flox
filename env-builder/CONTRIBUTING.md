...
- _Bug Fixes_ and _Optimizations_ which have no effect on the availability of
  ...
  + When users see that _patch_ was incremented they expect to update `phlox`
...
- _New Features_ and _Interfaces_ such as new subcommands/flags, new C++
  interfaces/class member variables defined in any `<phlox>/include/` file, or
  any change which does not otherwise effect backwards compatibility should
  increment _minor_ version.
  + When users see that _minor_ was incremented they expect to update `phlox`
...
- _Removed Features_ or _Breaking Changes_ such as removed subcommands/flags,
  removed interfaces/class member variables defined in any `<phlox>/include/`
  ...
  + When users see that _major_ version was incremented they know that they
    ...
    existing usage of `phlox` in their software.
...
These readers could care less if `phlox` is at version 3.2.1 or 30000.2.1!


### `phlox` Software Versioning
Updates to `phlox` version numbers are controlled by the text file
`<phlox>/cpp/version` ( in the repository root ).
This file is used to populate the CPP variable `FLOX_phlox_VERSION`, the `nix`
