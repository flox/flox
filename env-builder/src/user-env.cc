 ...
 * @file user-env.cc
 *
 * @brief Operations on "realized" environments.
 *
 *
 * -------------------------------------------------------------------------- */
...
queryInstalled( EvalState & state, const Path & userEnv )
{
    ...
    if ( pathExists( userEnv + "/manifest.json" ) )
      {
        throw Error(
          "profile '%s' is incompatible with 'nix-env'; please use "
          "'flox profile' instead"
        , userEnv
        );
      }
             ...
             )
{
    ...
    for ( auto & i : elems )
      {
        ...
        std::optional<StorePath> drvPath = keepDerivations ? i.queryDrvPath()
                                                           : std::nullopt;
        ...
        attrs.alloc( state.sOutPath ).mkString(
          state.store->printStorePath( i.queryOutPath() )
        );
        if ( drvPath )
          {
            attrs.alloc( state.sDrvPath ).mkString(
        ...
        for ( const auto & [m, j] : enumerate( outputs ) )
          {
            ( vOutputs.listElems()[m] =
                state.allocValue()
            )->mkString( j.first );
            auto outputAttrs = state.buildBindings( 2 );
            outputAttrs.alloc( state.sOutPath ).mkString(
              state.store->printStorePath( * j.second )
            );
    ...
    auto topLevelDrv = state.coerceToStorePath( aDrvPath.pos
                                              , * aDrvPath.value
                                              , context
                                              , ""
                                              );
    Attr & aOutPath( * topLevel.attrs->find( state.sOutPath ) );
    auto topLevelOut = state.coerceToStorePath( aOutPath.pos
                                              , * aOutPath.value
                                              , context
                                              , ""
                                              );
    ...
    if ( store2 )
      {
        ...
        if ( lockToken != lockTokenCur )
          {
            printInfo( "profile '%1%' changed while we were busy; restarting"
                     , profile
                     );
            return false;
          }
