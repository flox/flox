...
queryInstalled( EvalState & state, const Path & userEnv )
{
  ...
  if ( pathExists( userEnv + "/manifest.json" ) )
    {
      throw Error( "profile '%s' is incompatible with 'nix-env'; please use "
               ...
               const std::string & lockToken )
{
  ...
  for ( auto & i : elems )
    {
      ...
      std::optional<StorePath> drvPath
        = keepDerivations ? i.queryDrvPath() : std::nullopt;
      ...
      attrs.alloc( state.sOutPath )
        .mkString( state.store->printStorePath( i.queryOutPath() ) );
      if ( drvPath )
        {
          attrs.alloc( state.sDrvPath )
            .mkString( state.store->printStorePath( *drvPath ) );
        }
      ...
      for ( const auto & [m, j] : enumerate( outputs ) )
        {
          ...
          outputAttrs.alloc( state.sOutPath )
            .mkString( state.store->printStorePath( *j.second ) );
  ...
  auto manifestFile
    = state.store->addTextToStore( "env-manifest.nix", str.str(), references );
...
#include "buildenv.nix.gen.hh"
  ...
  auto   topLevelOut
    = state.coerceToStorePath( aOutPath.pos, *aOutPath.value, context, "" );
