...
// closure.c calls fopen, so its reference must be bound to the minimum GLIBC
// version like the other symbols in glibc-bindings.h. It lives here rather than
// in that shared header because the header is also included by sandbox.c, which
// *defines* its own fopen (the LD_PRELOAD interceptor). GNU as reads `.symver`
// differently in each case: where fopen is only referenced (here) it versions
// the reference, which is what we want; where fopen is defined (sandbox.c) it
// would instead version the definition, emitting a versioned alias of the
// interceptor — which defeats the interposition and needs a linker version
// script we don't use. Keeping it in this reference-only file avoids that.
