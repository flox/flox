            ...
            let
              ...
              callPackage =
                ...
                if final ? newScope then
                  ...
                  # a caller placed in `currentScope` -- e.g.
                  # `mkGlobalOverridesOverlay` binding `catalogs` to a throw
                  # -- wins even though `final` is nixpkgs' top-level self
                  # and can carry attributes this extension's own `prev`
                  # never had.
                  final.newScope (
                    currentScope
                    // {
