...
function verify_requisites() {
    ...
    # populate missing_reqs with keys in _output_requisites not in _allowed_requisites
...
function find_packages_in_env {
    ...
    while IFS=: read -r install_id output_path; do
        ...
        if [[ ! -e $ref_path ]]; then
            ...
            if [[ -e "$dev_path" ]]; then
                ...
                if [[ ${#_hint_pkgs[@]} -gt 0 ]]; then
                    local key_or_keys="${!_hint_pkgs[*]}"
                    if [[ ${#_hint_pkgs[@]} -gt 1 ]]; then
                       hint="consider adding one of the following to 'runtime-packages': $key_or_keys"
                    else
                       hint="consider adding package '$key_or_keys' to 'runtime-packages'"
                    fi
                else
                    hint="package not found in develop env"
                fi
                printf '%sHint: %s\n' "$_indent" "$hint"
            else
                printf '%sNot found in Flox environment\n' "$_indent"
            fi
            echo
        fi
