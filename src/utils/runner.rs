...
use crate::environment::{get_nix_cmd, build_flox_env};
...
impl CommandRunner {
    ...
    pub async fn run_in_nix(cmd: &str, args: &Vec<&str>) -> Result<String> {
        ...
        if !nix_err_response.is_empty() {
            println!("Error in nix response, {}, {}", nix_err_response,nix_err_response.len());
            Err(anyhow!("FXXXX: Error in nix response, {}, {}",nix_err_response,nix_err_response.len()))
        } else {
