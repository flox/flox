...
@test "flox activate works with npm" {
  ...
  _FLOX_USE_CATALOG_MOCK="$GENERATED_DATA/init/node_npm/resp.yaml" \
...
@test "auto init matches yarn version to yarn 1.x" {
  ...
  _FLOX_USE_CATALOG_MOCK="$GENERATED_DATA/init/yarn_1x/resp.yaml" \
    run "$FLOX_BIN" init --auto-setup
...
@test "auto init installs nodejs major version package" {
  ...
  _FLOX_USE_CATALOG_MOCK="$GENERATED_DATA/init/nodejs_20/resp.yaml" \
    run "$FLOX_BIN" init --auto-setup
  assert_output --partial "'nodejs' installed"
  run "$FLOX_BIN" list
...
@test "auto init installs nodejs version range package" {
  ...
  _FLOX_USE_CATALOG_MOCK="$GENERATED_DATA/init/nodejs_lt_24/resp.yaml" \
    run "$FLOX_BIN" init --auto-setup
  assert_output --partial "'nodejs' installed"
  run "$FLOX_BIN" list
...
@test "auto init matches yarn version to yarn-berry" {
  ...
  _FLOX_USE_CATALOG_MOCK="$GENERATED_DATA/init/yarn_berry/resp.yaml" \
    run "$FLOX_BIN" init --auto-setup
  assert_output --partial "'yarn' installed"
  run "$FLOX_BIN" list
...
@test "install krb5 with node" {
  ...
  cat "$INPUT_DATA/manifests/krb5_prereqs.toml" | _FLOX_USE_CATALOG_MOCK="$GENERATED_DATA/env/krb5_prereqs/resp.yaml" "$FLOX_BIN" edit -f -
