...
no_build_after_pull_copy() {
  # No build if latest generation specified explicitly
  "$FLOX_BIN" pull --remote owner/name --copy --generation 2
  ...
  "$FLOX_BIN" pull --remote owner/name --copy --generation 1
  ...
  "$FLOX_BIN" pull --remote owner/name --copy
...
# Using a FloxHub with floxmeta generated by 1.3.17, verify that we don't do a
# build after a pull --copy
#
# I confirmed this is a valid test by doing:
# > test_data/manually_generated/generate-floxmeta.sh v1.3.16 $PWD/cli/tests
# Then I changed 1.3.17 to 1.3.16 below and confirmed the test fails
@test "no build for an activate after a 'pull --copy' of an old environment" {
  rm -rf "$FLOXHUB_FLOXMETA_DIR"
  mkdir "$FLOXHUB_FLOXMETA_DIR"
  tar -C "$FLOXHUB_FLOXMETA_DIR" -xzf "$MANUALLY_GENERATED/with-v1.3.17/floxmeta.tar.gz"
