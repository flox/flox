...
# bats test_tags=pull:expired
# When pulling a new environment owned by the current user with an expired token,
# the expired token warning should be shown. With the mock floxhub (file:// URLs),
# the pull succeeds because no real authentication is required.
# This test verifies the warning appears and the user identity is preserved.
@test "pull new: expired token shows warning when pulling own environment" {
  # Set up floxhub for "test" owner (matching the expired token's handle)
  floxhub_setup "test"

  # Create a dummy environment owned by "test"
  make_dummy_env "test" "expired-test"

  # Clear the local floxmeta cache to force a fresh pull
  rm -rf "$FLOX_DATA_DIR/meta/test"

  # Set an expired token (handle: "test", exp: 2024-01-01T00:00:00+00:00)
  ...
  run "$FLOX_BIN" pull test/expired-test
  # With the mock floxhub, the pull succeeds because file:// URLs don't require auth.
  # The key verification is that the expired token warning appears.
  assert_output --partial 'Your FloxHub token has expired.'
}

# bats test_tags=pull:expired
# When pulling updates to an existing managed environment with an expired token,
# the expired token warning should be shown.
@test "pull updates: expired token shows warning when pulling own environment" {
  # Set up floxhub for "test" owner (matching the expired token's handle)
  floxhub_setup "test"

  # Create and pull an environment owned by "test"
  make_dummy_env "test" "expired-update-test"

  # Pull the environment first with valid token
  "$FLOX_BIN" pull test/expired-update-test

  # Now set an expired token and try to pull updates
  export FLOX_FLOXHUB_TOKEN="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJodHRwczovL2Zsb3guZGV2L2hhbmRsZSI6InRlc3QiLCJleHAiOjE3MDQwNjM2MDB9.-5VCofPtmYQuvh21EV1nEJhTFV_URkRP0WFu4QDPFxY"

  run "$FLOX_BIN" pull
  # With the mock floxhub, pull succeeds because file:// URLs don't require auth.
  # The key verification is that the expired token warning appears.
  assert_output --partial 'Your FloxHub token has expired.'
}

# bats test_tags=pull:expired
# When pulling an environment NOT owned by the current user with an expired token,
# we should NOT require re-authentication (the environment might be public).
@test "pull: expired token does not prompt re-auth for other user's environment" {
  # Create a dummy environment owned by "other-owner" (NOT matching the expired token's handle "test")
  floxhub_setup "other-owner"
  make_dummy_env "other-owner" "public-env"

  # Clear the local floxmeta cache
  rm -rf "$FLOX_DATA_DIR/meta/other-owner"

  # Set an expired token with handle "test"
  export FLOX_FLOXHUB_TOKEN="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJodHRwczovL2Zsb3guZGV2L2hhbmRsZSI6InRlc3QiLCJleHAiOjE3MDQwNjM2MDB9.-5VCofPtmYQuvh21EV1nEJhTFV_URkRP0WFu4QDPFxY"

  # Should succeed because the environment is "public" (owned by different user)
  # and we use a mock floxhub that doesn't require auth
  run "$FLOX_BIN" pull other-owner/public-env < /dev/null
  assert_success
  # Should show the warning but not fail with re-auth prompt
  assert_output --partial 'Your FloxHub token has expired.'
}
