...
pub fn run(args: Cli) -> Result<(), Error> {
    ...
    let should_reap = Arc::new(AtomicBool::new(false));
    signal_hook::flag::register(SIGCHLD, Arc::clone(&should_reap))
        .context("failed to set SIGCHLD signal handler")?;
