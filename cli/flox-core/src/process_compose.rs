...
///
/// Bounded at every step, because this runs on interactive paths.
fn get(socket: &Path, path: &str) -> Option<String> {
    ...
    // Bounded by the read timeout, so a manager that accepts and then goes
    // quiet cannot hang the caller. The cap keeps a confused peer from
    // streaming at us.
