...
/// On-disk JSONL file used by the canonical event pipeline.
pub const EVENTS_BUFFER_FILE_NAME: &str = "canonical-events-v1.json";
const EVENTS_LOCK_FILE_NAME: &str = "canonical-events-lock";
...
impl EventsBuffer {
    ...
    pub fn read(data_dir: &Path) -> Result<Self> {
        std::fs::create_dir_all(data_dir).with_context(|| {
            format!(
                "Could not create canonical events buffer directory at {}",
                data_dir.display()
            )
        })?;

        let mut events_lock = LockFile::open(&data_dir.join(EVENTS_LOCK_FILE_NAME))
            .context("Could not open canonical events lock file")?;
        events_lock
            .lock()
            .context("Could not lock canonical events buffer")?;
        ...
        let mut events_buffer_file = events_buffer_file_options
            ...
            .with_context(|| {
                format!(
                    "Could not open canonical events buffer file at {}",
        ...
        events_buffer_file
            ...
            .context("Could not read canonical events buffer file")?;
        ...
        let buffer = serde_json::Deserializer::from_str(&buffer_json)
            ...
            .filter_map(|event| match event {
                Ok(event) => Some(event),
                Err(err) => {
                    debug!(error = %err, "Skipping unreadable canonical event buffer entry");
                    None
                },
            })
            .collect();
    ...
    pub fn overwrite_file(&mut self) -> Result<()> {
        self.storage
            .set_len(0)
            .context("Could not truncate canonical events buffer file")?;

        for event in &self.buffer {
            self.storage
                .write_all(serde_json::to_string(event)?.as_bytes())
                .context("Could not write canonical event to buffer file")?;
            self.storage
                .write_all(b"\n")
                .context("Could not write canonical event buffer newline")?;
        }

        self.storage
            .flush()
            .context("Could not flush canonical events buffer file")?;
        self.storage
            .sync_data()
            .context("Could not sync canonical events buffer file")?;
        Ok(())
    }

    /// Push a new event and sync it to disk before returning.
    pub fn push(&mut self, event: Event) -> Result<()> {
        debug!(event_id = %event.event_id, "Pushing event to canonical events buffer");
    ...
    fn append_new_event(&mut self) -> Result<()> {
        let event = self
            .buffer
            .back()
            .expect("buffer has a just-pushed canonical event");
        self.storage
            .write_all(serde_json::to_string(event)?.as_bytes())
            .context("Could not write new canonical event to buffer file")?;
        self.storage
            .write_all(b"\n")
            .context("Could not write canonical event buffer newline")?;
        self.storage
