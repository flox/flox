...
coordinator-reported status appears.*/
    ...
    ///Return builds created at or after this time (inclusive). Either a relative duration matching [0-9]+[smhdwy] (e.g. '7d') or an absolute ISO 8601 timestamp; a timestamp without a UTC offset is read as UTC.
    ///
    /// <details><summary>JSON schema</summary>
    ///
    /// ```json
    ///{
    ///  "title": "Since",
    ///  "description": "Return builds created at or after this time (inclusive). Either a relative duration matching [0-9]+[smhdwy] (e.g. '7d') or an absolute ISO 8601 timestamp; a timestamp without a UTC offset is read as UTC.",
    ///  "type": "string",
    ///  "minLength": 1
    ///}
    /// ```
    ...
    #[derive(::serde::Serialize, Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
    #[serde(transparent)]
    pub struct Since(::std::string::String);
    impl ::std::ops::Deref for Since {
        type Target = ::std::string::String;
        fn deref(&self) -> &::std::string::String {
            &self.0
        }
    }
    impl ::std::convert::From<Since> for ::std::string::String {
        fn from(value: Since) -> Self {
            value.0
        }
    }
    impl ::std::convert::From<&Since> for Since {
        fn from(value: &Since) -> Self {
            value.clone()
        }
    }
    ...
    impl ::std::convert::TryFrom<&str> for Since {
        type Error = self::error::ConversionError;
        fn try_from(
            value: &str,
        ) -> ::std::result::Result<Self, self::error::ConversionError> {
    ...
    impl ::std::convert::TryFrom<&::std::string::String> for Since {
        type Error = self::error::ConversionError;
        fn try_from(
            value: &::std::string::String,
        ) -> ::std::result::Result<Self, self::error::ConversionError> {
    ...
    impl<'de> ::serde::Deserialize<'de> for Since {
        fn deserialize<D>(deserializer: D) -> ::std::result::Result<Self, D::Error>
        where
            D: ::serde::Deserializer<'de>,
        {
