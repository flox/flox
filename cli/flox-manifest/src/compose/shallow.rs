...
use crate::parsed::common::{Allows, Containerize, Hook, Include, SemverOptions, Vars};
...
// merge_options operates on the latest schema's Options (which carries
// `activate.add-sbin`), so composing environments preserves the field.
...
impl ShallowMerger {
    ...
    fn merge_install(
        ...
        Ok((Install::from(merged), warnings))
    }
...
impl ManifestMergeTrait for ShallowMerger {
    fn merge(
        &self,
        low_priority: &ManifestLatest,
        high_priority: &ManifestLatest,
    ) -> Result<(ManifestLatest, Vec<Warning>), MergeError> {
