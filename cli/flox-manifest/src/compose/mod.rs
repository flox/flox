...
pub struct CompositeManifest {
    ...
    pub composer: ManifestLatest,

    /// (name, manifest)
    /// The order is significant; later manifests have higher priority.
    #[cfg_attr(
        any(test, feature = "tests"),
        proptest(strategy = "proptest::collection::vec(any::<(String, ManifestLatest)>(), 0..=2)")
    )]
    pub deps: Vec<(String, ManifestLatest)>,
}
...
impl CompositeManifest {
    ...
    ) -> Result<(ManifestLatest, Vec<WarningWithContext>), MergeError> {
    ...
    fn include_declared_plugin_hooks_are_ignored_with_warning() {
        ...
        let dep_with_hooks = ManifestLatest {
            plugin_hooks: Some(PluginHooks {
                ...
                env: vec!["sneaky-plugin".to_string()],
                ..Default::default()
            }),
            ..Default::default()
        };
