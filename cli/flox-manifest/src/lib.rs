...
/// The outcome of [`Manifest::parse_toml_typed_or_bumped`].
#[derive(Debug, Clone)]
pub enum ParsedManifest {
    ...
    /// to the minimum schema version that accepts all fields present.
    Bumped {
...
impl Manifest<Init> {
    ...
    /// minimum schema version that accepts the fields present if the stated
    /// version fails to validate.
    ///
    /// Returns `AsStated` when the manifest validates at its stated schema
    /// version, or `Bumped` when it fails at the stated version but succeeds
    /// after the schema-version key is rewritten to the lowest candidate `v`
    /// where `v > stated` and the manifest validates.
    ///
    /// Returns the original stated-version error (never `AsStated`) when the
    /// stated version is already `latest()` (making the candidate set empty),
    /// or when no candidate version validates either. Returns the TOML parse
    /// error when the input is not well-formed TOML (a structural error, not a
    /// schema mismatch).
    pub fn parse_toml_typed_or_bumped(s: impl AsRef<str>) -> Result<ParsedManifest, ManifestError> {
        ...
        match untyped.validate() {
            ...
            Err(stated_err) => {
                ...
                // Try each version strictly above the stated one in ascending
                // order. The first one that validates is the minimum bump.
...
#[cfg(test)]
mod parse_toml_typed_or_bumped_tests {
    use indoc::indoc;

    use super::*;
    use crate::test_helpers::{with_latest_schema, with_schema};

    /// A `version = 1` manifest that uses `outputs` (introduced in 1.10.0)
    /// must bump to exactly V1_10_0, not to latest.
    #[test]
    fn legacy_version_with_outputs_bumps_to_v1_10_0() {
        ...
        match result {
            ParsedManifest::Bumped { from, to, .. } => {
                ...
                assert_eq!(to, KnownSchemaVersion::V1_10_0);
            },
    ...
    /// A `schema-version = "1.10.0"` manifest that uses `minimum-cli-version`
    /// (introduced in 1.11.0) must bump to exactly V1_11_0, not to latest.
    /// This is the key regression guard: the bump must be the minimum, not
    /// always latest.
    #[test]
    fn v1_10_0_with_minimum_cli_version_bumps_to_v1_11_0() {
        let contents = with_schema(KnownSchemaVersion::V1_10_0, indoc! {r#"
            [minimum-cli-version]
            version = "1.0.0"
            reason = "uses outputs"
        ...
        match result {
            ParsedManifest::Bumped { from, to, .. } => {
                assert_eq!(from, KnownSchemaVersion::V1_10_0);
                assert_eq!(to, KnownSchemaVersion::V1_11_0);
            },
            ParsedManifest::AsStated(_) => {
                panic!("expected Bumped, got AsStated");
            },
    ...
    fn already_latest_invalid_field_returns_err() {
        ...
        "#});
        assert!(Manifest::parse_toml_typed_or_bumped(&contents).is_err());
    }
