...
/// Which installed plugins participate in which lifecycle hooks.
///
/// Values name plugins, i.e. keys of `[plugins.<name>]` and install ids of
/// the packages that ship the hook files. Declaring a hook here is what
/// arms it: a hook file shipped by a package that is not declared is
/// ignored. `session-wrap` is single-valued because exactly one plugin may
/// wrap the activation — two wrappers are unrepresentable rather than
/// being an activation-time error.
#[skip_serializing_none]
...
pub struct PluginHooks {
    ...
    /// Plugins whose `env` hooks contribute environment variables at
    /// activation start and on every attach.
    #[serde(default)]
    #[serde(skip_serializing_if = "Vec::is_empty")]
    #[cfg_attr(
        any(test, feature = "tests"),
        proptest(strategy = "proptest::collection::vec(alphanum_string(5), 0..3)")
    )]
    pub env: Vec<String>,
    /// Plugins whose `sidecar` hooks run as executive-supervised processes
    /// for the lifetime of the activation.
    #[serde(default)]
    #[serde(skip_serializing_if = "Vec::is_empty")]
    #[cfg_attr(
        any(test, feature = "tests"),
...
impl PluginHooks {
    ...
    pub fn is_empty(&self) -> bool {
        self.session_wrap.is_none() && self.env.is_empty() && self.sidecar.is_empty()
    }
}
