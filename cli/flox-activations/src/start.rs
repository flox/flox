...
fn signal_new_process_compose(
    ...
    // Taken before signalling, so it is older than the log the executive is
    // about to name and newer than every log already there.
    let signalled_at = log_timestamp_now().ok();
...
fn socket_not_ready_message(
    log_dir: &Path,
    waited: Duration,
    signalled_at: Option<&str>,
) -> String {
    let log = latest_services_log(log_dir, signalled_at);
