...
pub fn generate_deactivate_script(shell: ShellWithPath, writer: &mut impl Write) -> Result<()> {
    let shell_type = Shell::from(shell);

    match shell_type {
        Shell::Bash => {
            let action: Action<BashStartupArgs> = Action::Deactivate;
            generate_bash_profile_commands(&action, writer)
        },
        Shell::Zsh => {
            let action: Action<ZshStartupArgs> = Action::Deactivate;
            generate_zsh_profile_commands(&action, writer)
        },
        Shell::Fish => {
            let action: Action<FishStartupArgs> = Action::Deactivate;
            generate_fish_profile_commands(&action, writer)
        },
        Shell::Tcsh => {
            let action: Action<TcshStartupArgs> = Action::Deactivate;
