...
/// Nothing is joined onto an empty base: that would leave a leading
/// separator, an empty element that `PATH` and `CPATH` read as the current
/// directory. The recorded old value is never used as the base, since it
/// belongs to the recording shell.
fn apply_growth(record: &TraceRecord, current: Option<String>) -> String {
    ...
    let elements: Vec<&str> = elements.split(sep).collect();
    if prepend {
        list.retain(|element| !elements.contains(element));
