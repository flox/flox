...
/// Mutable broker state shared across connections: the session grant globs
/// and the pending queue. Behind a single mutex because verdicts are
/// infrequent (one per out-of-policy access, most of which the engine caches)
/// and the critical section is tiny.
#[derive(Debug)]
pub struct BrokerState {
    /// Session grant globs (seeded from grants.toml at broker start).
    grants: Vec<String>,
...
impl BrokerState {
    /// Build broker state from the initial set of grant globs.
    ...
    pub fn decide(&mut self, request: &VerdictRequest) -> VerdictResponse {
        if let Some(glob) = first_matching_grant(&self.grants, &request.path) {
    ...
    /// Number of distinct pending requests, for tests and a later review CLI.
    #[allow(dead_code)]
...
mod tests {
    ...
    fn fs_request_line_round_trips_through_decide_line() {
        ...
        let line = r#"{"v":1,"kind":"fs","op":"read","path":"/home/dev/x","raw":"~/x","pid":7,"exe":""}"#;
        let response = decide_line(line, &state);
