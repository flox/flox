...
//! `hook.on-activate` left it (replayed from the start's env diff), with
...
fn run_hook_script(
    ...
    // Replay the end-of-activation environment so the hook sees variables
    // exported by hook.on-activate. A start that never wrote its env diff
    // never finished hook.on-activate, so its bookend never opened; skip.
    let vars_from_env = VarsFromEnvironment::get()?;
    let start_diff = StartDiff::from_files(&start_state_dir)
        .context("start has no environment diff; hook.on-activate never completed")?;
...
mod test {
    ...
    use super::*;
    use crate::start_diff::{ENV_DIFF_END_JSON, ENV_DIFF_START_JSON};
    ...
    fn setup_start(tmp: &TempDir, script: Option<&str>) -> (StartIdentifier, PathBuf, PathBuf) {
        ...
        start_id
            ...
            .unwrap();
        // Env captured around hook.on-activate: it exported ON_ACTIVATE_VAR.
        std::fs::write(start_state_dir.join(ENV_DIFF_START_JSON), "{}").unwrap();
        std::fs::write(
            start_state_dir.join(ENV_DIFF_END_JSON),
            r#"{"ON_ACTIVATE_VAR": "from-on-activate"}"#,
        )
        .unwrap();
