...
pub(crate) mod test_helpers {
    ...
    use super::*;
    use crate::models::lockfile;
    use crate::models::manifest::typed::{Inner, PackageDescriptorCatalog};
    ...
    pub(crate) fn locked_package_catalog_from_mock(
    ...
    pub(crate) fn locked_package_catalog_from_mock_all_systems(
        install_id: &str,
        mock_lockfile_path: impl AsRef<Path>,
    ) -> (PackageDescriptorCatalog, Vec<LockedPackageCatalog>) {
        let lockfile =
            lockfile::Lockfile::read_from_file(&CanonicalPath::new(mock_lockfile_path).unwrap())
                .expect("failed to read lockfile");
        let ManifestPackageDescriptor::Catalog(pd) = lockfile
            .manifest
            .install
            .inner()
            .get(install_id)
...
#[cfg(test)]
pub(crate) mod realise_nixpkgs_tests {

    use test_helpers::{buildenv_instance, locked_package_catalog_from_mock};
