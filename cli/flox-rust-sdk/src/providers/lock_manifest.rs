...
/// A provider responsible for operations responsible for locking manifests,
/// which includes extracting and organizing constraints from the manifest
/// and an optionally pre-existing lockfile.
...
impl LockManifest {
    ...
    fn seed_mapping(
        ...
        ManifestError,
    > {
        if let Some(lockfile) = maybe_seed {
            Self::make_seed_mapping(lockfile)
        } else {
            Ok(HashMap::new())
        }
    }
    ...
    fn collect_resolution_package_groups(
        ...
        let pkg_systems = |install_id: &str, pkg: &PackageDescriptorCatalog| {
            let available_systems = manifest_systems.unwrap_or(default_systems);
            let package_systems = pkg.systems.as_deref();
            for system in package_systems.into_iter().flatten() {
                if !available_systems.contains(system) {
                    return Err(ResolveError::SystemUnavailableInManifest {
                        ...
                        enabled_systems: available_systems.iter().map(|s| s.to_string()).collect(),
                    });
                }
            }
            package_systems
        ...
        for (group_name, pkgs) in pkgs_by_group.iter() {
            ...
            for (id, desc) in pkgs.iter() {
                let systems = pkg_systems(id.as_str(), desc)?;
    ...
    fn merge_manifest(
        ...
        let deps = locked_includes
            .iter()
            .map(|include| {
                include
                    .manifest
                    .migrate_typed_only(None)
                    .map(|manifest| (include.name.clone(), manifest.as_latest_schema().clone()))
            })
            .collect::<Result<Vec<_>, _>>()
            .map_err(RecoverableMergeError::Migration)?;
        ...
        let compose = Compose {
            composer: manifest.as_typed_only(), // this always stores a ManifestLatest
    ...
    > {
        ...
        Ok(seed
            .packages
            .iter()
            .filter_map(move |locked| {
                let system = locked.system().as_str();
                let install_id = locked.install_id();
                let descriptor = manifest.install.inner().get(locked.install_id()).cloned()?;
    ...
    fn split_fully_locked_groups(
        ...
        seed_lockfile: Option<&Lockfile>,
    ) -> Result<(Vec<LockedPackage>, Vec<PackageGroup>), ResolveError> {
        let seed_locked_packages = Self::seed_mapping(seed_lockfile)?;
        ...
        Ok((already_locked_packages, groups_to_lock))
    }
    ...
    fn locked_packages_from_resolution<'manifest>(
        manifest: &'manifest ManifestLatest,
        ...
        let locked_pkg_iter = groups
            ...
            .filter_map(|resolved_pkg| {
                manifest
                    .catalog_descriptor_with_id(&resolved_pkg.install_id)
    ...
    fn determine_invalid_systems(
        ...
        let pkg_descriptor = manifest
            .catalog_descriptor_with_id(&r_msg.install_id)
    ...
    fn split_locked_flake_installables(
        ...
        seed_lockfile: Option<&Lockfile>,
    ) -> Result<(Vec<LockedPackage>, Vec<FlakeInstallableToLock>), ResolveError> {
    ...
    pub fn unlock_specified_packages_or_groups(lockfile: &mut Lockfile, groups_or_iids: &[&str]) {
        if groups_or_iids.is_empty() {
            lockfile.packages = Vec::new();
        } else {
            lockfile.packages = std::mem::take(&mut lockfile.packages)
                .into_iter()
                .filter(|package| {
...
mod tests {
    ...
    static TEST_TYPED_MANIFEST_LATEST: LazyLock<ManifestLatest> =
        LazyLock::new(|| toml_edit::de::from_str(TEST_MANIFEST_LATEST_CONTENTS).unwrap());
    ...
    fn make_params_smoke() {
        let manifest = &*TEST_TYPED_MANIFEST_LATEST;

        let params = LockManifest::collect_resolution_package_groups(manifest, None)
            .unwrap()
            .collect::<Vec<_>>();
    ...
    fn make_params_seeded() {
        let mut manifest = TEST_TYPED_MANIFEST_LATEST.clone();
        ...
        let actual_params = LockManifest::collect_resolution_package_groups(
            &manifest,
            Some(&*TEST_LOCKED_MANIFEST),
        )
        .unwrap()
        .collect::<Vec<_>>();
    ...
    fn make_params_seeded_unchanged() {
        ...
        let actual_params =
            LockManifest::collect_resolution_package_groups(&manifest_before, Some(&seed))
                .unwrap()
                .collect::<Vec<_>>();
    ...
    fn make_params_seeded_unlock_if_invalidated() {
        ...
        let mut manifest_after = ManifestLatest::default();
        manifest_after
            .install
            .inner_mut()
    ...
    /// [LockManifest::collect_resolution_package_groups] generates [FlakeInstallableToLock]
    ...
    fn ungroup_response() {
        ...
        let locked_packages =
            LockManifest::locked_packages_from_resolution(manifest, groups.clone())
                .unwrap()
                .collect::<Vec<_>>();
    ...
    fn unlock_by_iid() {
        ...
        LockManifest::unlock_specified_packages_or_groups(&mut lockfile, &[&foo_iid, &baz_iid]);
    ...
    fn unlock_by_group() {
        ...
        LockManifest::unlock_specified_packages_or_groups(&mut lockfile, &["group"]);
    ...
    fn unlock_by_iid_and_group() {
        ...
        LockManifest::unlock_specified_packages_or_groups(&mut lockfile, &[&foo_iid]);
    ...
    fn unlock_by_iid_noop_if_already_unlocked() {
        ...
        LockManifest::unlock_specified_packages_or_groups(&mut seed, &["not in here"]);
    ...
    async fn locking_unknown_message() {
        ...
        let locked_manifest = LockManifest::resolve_manifest(
    ...
    fn collect_package_groups_defaults_to_four_systems() {
        let manifest_str = indoc! {r#"
            ...
            hello_install_id.pkg-path = "hello"
        "#};
        let manifest = Manifest::parse_and_migrate(manifest_str, None).unwrap();
        let package_groups: Vec<_> =
            LockManifest::collect_resolution_package_groups(manifest.as_latest_schema(), None)
                .unwrap()
                .collect();
    ...
    fn test_split_out_fully_locked_packages() {
        ...
        let groups =
            LockManifest::collect_resolution_package_groups(&manifest, Some(&locked)).unwrap();

        let (fully_locked, to_resolve): (Vec<_>, Vec<_>) =
            LockManifest::split_fully_locked_groups(groups, Some(&locked)).unwrap();
    ...
    fn drop_packages_for_removed_systems() {
        ...
        let groups = LockManifest::collect_resolution_package_groups(&manifest, Some(&locked))
            .unwrap()
            .collect::<Vec<_>>();
    ...
    fn split_out_locked_installables() {
        ...
        let flake_installables = LockManifest::collect_flake_installables(&manifest);

        let (locked, to_resolve): (Vec<_>, Vec<_>) =
            LockManifest::split_locked_flake_installables(flake_installables, Some(&locked))
                .unwrap();
    ...
    async fn lock_manifest_noop_if_fully_locked() {
        ...
        let locked_manifest = LockManifest::lock_manifest(
            &flox,
    ...
    proptest! {
        ...
        fn lock_manifest_noop_if_locked_without_install_section((flox, tempdir, environments_to_include) in generate_path_environments_without_install_or_include(3)) {
            ...
            let inner_manifest = manifest.as_latest_schema_mut();
            inner_manifest.include = Include {
            ...
            let lockfile_2 =
                LockManifest::lock_manifest(&flox, &manifest.as_migrated_typed_only(), Some(&lockfile), &mock_include_fetcher())
                    .block_on()
                    .unwrap();
    ...
    /// [LockManifest::merge_manifest] fetches an included environment when it is
    /// not already locked
    #[test]
    ...
    /// [LockManifest::merge_manifest] preserves precedence of includes
    #[test]
    ...
    async fn re_merge_after_editing_dep(modify_include_descriptor: bool) {
        ...
        let mut manifest_contents = with_latest_schema(indoc! {r#"
            ...
            "#});
        let mut manifest = mk_test_manifest_from_contents(manifest_contents);
        ...
        let dep1_manifest_contents = with_latest_schema(indoc! {r#"
            ...
            foo = "dep1"
            "#});
        let dep1_manifest = mk_test_manifest_from_contents(&dep1_manifest_contents);
        ...
        let dep1_edited_manifest_contents = with_latest_schema(indoc! {r#"
            [vars]
            foo = "dep1 edited"
            "#});
        let dep1_edited_manifest = mk_test_manifest_from_contents(&dep1_edited_manifest_contents);
    ...
    async fn merge_manifest_removes_stale_locked_includes() {
        ...
        assert_eq!(&merged, manifest.as_latest_schema());

        assert!(compose.is_none());
    }

    /// [LockManifest::merge_manifest] errors if locked include names are not unique
    #[test]
