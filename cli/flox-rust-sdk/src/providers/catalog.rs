...
pub enum Client<'server> {
    Catalog(CatalogClient<'server>),
    Mock(MockClient),
}
...
#[allow(dead_code)]
enum MockGuard<'server> {
    None,
    Record(MockRecorder<'server>),
    Replay(MockServer),
}
impl Debug for MockGuard<'_> {
    fn fmt(&self, _: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Ok(()) // TODO
    }
}

struct MockRecorder<'server> {
    path: PathBuf,
    _server: MockServer,
    recording: Option<httpmock::Recording<'server>>,
}
impl Drop for MockRecorder<'_> {
    fn drop(&mut self) {
        // let recorded = self
        //     .recording
...
#[derive(Debug)]
pub struct CatalogClient<'server> {
    client: APIClient,
    config: CatalogClientConfig,
    _mock_guard: MockGuard<'server>,
}

impl<'server> CatalogClient<'server> {
    pub fn new(config: CatalogClientConfig) -> Self {
        ...
        Self {
            client: Self::create_client(&modified_config),
            config: modified_config.clone(),
            _mock_guard: mock_guard,
        }
    }

    fn configure_mocks(config: CatalogClientConfig) -> (MockGuard<'server>, CatalogClientConfig) {
        match &config.mock_mode {
            ...
            CatalogMockMode::Record(path) => {
                ...
                // if let MockGuard::Record(recorder) = &mut guard {
                //     let recording = recorder._server.record(|rule| {
                //         rule.record_request_header("User-Agent").filter(|when| {
                //             when.any_request();
                //         });
                //     });
    ...
    fn start_recording<'b: 'server>(&'b mut self) {
        if let MockGuard::Record(ref mut recorder) = self._mock_guard {
            recorder.recording = Some(recorder._server.record(|rule| {
                rule.record_request_header("User-Agent").filter(|when| {
                    when.any_request();
                });
            }));
...
impl ClientTrait for CatalogClient<'_> {
