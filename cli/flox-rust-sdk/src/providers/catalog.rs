...
/// Wrapper around `semver::VersionReq` which attempts to normalize semver
/// strings in a format that the catalog supports.
#[derive(Debug, Error)]
struct SemverNormalized(VersionReq);

impl TryFrom<String> for SemverNormalized {
    type Error = semver::Error;

    fn try_from(s: String) -> Result<Self, Self::Error> {
        let version = VersionReq::parse(&s)?;
        Ok(SemverNormalized(version))
    }
}

// Forked from:
// https://github.com/dtolnay/semver/blob/1.0.26/src/display.rs#L33-L46
impl std::fmt::Display for SemverNormalized {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.0.comparators.is_empty() {
            return f.write_str("*");
        }
        for (i, comparator) in self.0.comparators.iter().enumerate() {
            if i > 0 {
                // This differs from upstream by omitting spaces in the separators.
                f.write_str(",")?;
            }
            write!(f, "{}", comparator)?;
        }
...
impl ClientTrait for CatalogClient {
    ...
    async fn resolve(
        ...
        let semver_normalized_package_groups = package_groups
            .into_iter()
            .map(|group| {
                let descriptors = group
                    .descriptors
                    .into_iter()
                    .map(|descriptor| PackageDescriptor {
                        version: descriptor.version.map(|version| {
                            SemverNormalized::try_from(version.clone())
                                .map_or(version, |semver| semver.to_string())
                        }),
                        ..descriptor
                    })
                    .collect();
            ...
            .collect::<Vec<PackageGroup>>();

        let package_groups = api_types::PackageGroups {
            items: semver_normalized_package_groups
                .into_iter()
...
mod tests {
    ...
    fn test_semver_normalized() {
        ...
        for (input, expected) in test_cases {
            let actual = match SemverNormalized::try_from(input.to_string()) {
                Ok(semver) => semver.to_string(),
                Err(_) => panic!("Failed to parse semver for {input}"),
            };
            assert_eq!(actual, expected, "Didn't get expected semver for {input}");
        }
    }

    #[test]
    fn test_semver_unmodified() {
        let test_cases = vec!["3.*", "=3.11", "^3.11", "~3.11", "^3.11", ">=3.11,<3.12"];
        for input in test_cases {
            let actual = match SemverNormalized::try_from(input.to_string()) {
                Ok(semver) => semver.to_string(),
                Err(_) => panic!("Failed to parse semver for {input}"),
            };
            assert_eq!(actual, input, "Didn't get expected semver for {input}");
        }
    }

    #[test]
    fn test_semver_normalized_unparseable() {
        let test_cases = vec![
            // Python, supported by catalog.
            "==3.11",
            "~=3.11",
            // NodeJS, not currently supported by catalog, but can be.
            ">=3.11 <3.12",
            "3.11 - 3.12",
        ];
        for input in test_cases {
            let actual = SemverNormalized::try_from(input.to_string());
            assert!(actual.is_err(), "Expected error for {input}");
        }
    }
