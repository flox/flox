...
use flox_manifest::parsed::latest::{ServiceShutdown, Services};
#[cfg(test)]
...
pub struct ProcessShutdown {
    ...
    /// How long to wait for the service to shut down — whether via a shutdown
    /// command or a signal — before sending SIGKILL. When unset,
    /// process-compose applies its own default of 10 seconds.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    /// The message names the socket, so a capture that swallows the trailing
    /// colon reports a path that cannot exist.
    #[test]
