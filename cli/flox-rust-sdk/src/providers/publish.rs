    ...
    Builder: ManifestBuilder,
{
    fn publish(
        ...
        let _env_meta = match environment {
            ...
            PublishEnvironment::Path(env) => Ok(check_environment_metadata(env)),
        };

        // How to access "Builder" from obove?
        let _build_meta = check_builder_metadata(builder);
...
fn check_builder_metadata(_builder: impl ManifestBuilder) -> Result<CheckedBuildMetadata, String> {
...
fn check_environment_metadata(
    _environment: PathEnvironment,
) -> Result<CheckedEnvironmentMetadata, String> {
