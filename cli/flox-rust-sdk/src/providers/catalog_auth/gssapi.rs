//! GSSAPI/Kerberos authentication strategy
...
/// GSSAPI/Kerberos authentication strategy
///
/// Uses Kerberos tickets to generate SPNEGO tokens for HTTP Negotiate authentication.
pub struct GssapiAuthStrategy;

impl AuthStrategy for GssapiAuthStrategy {
        ...
        match Self::generate_gssapi_token(&config.catalog_url) {
            Ok(token) => {
                ...
                if let Ok(value) = HeaderValue::from_str(&auth_value) {
                    ...
                    tracing::warn!("Failed to create header value from GSSAPI token");
                }
            },
            Err(e) => {
