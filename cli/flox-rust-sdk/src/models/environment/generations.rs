...
impl AllGenerationsMetadata {
    ...
    pub fn add_generation(
        ...
        //
        // Keys should all be numbers, but if they aren't we provide a default value.
        let next_generation = self.generations.keys().cloned().max().unwrap_or_default();
    ...
    pub fn switch_generation(
        ...
        let previous_generation = self.current_gen;

        if Some(next_generation) == previous_generation {
            return Err(GenerationsError::RollbackToCurrentGeneration);
        }
        ...
        next_generation_metadata.last_active = Some(Utc::now());
...
mod tests {
    ...
    fn default_add_generation_option() -> AddGenerationOptions {
        AddGenerationOptions {
            author: AUTHOR.into(),
    ...
    mod metadata {
        ...
        use crate::models::environment::generations::tests::default_add_generation_option;
        ...
        fn add_generation_adds_metadata_and_history() {
            ...
            let options = default_add_generation_option();
        ...
        fn generation_counter_is_correctly_increased() {
            let mut metadata = AllGenerationsMetadata::default();
            let (first_generation, _, _) = metadata.add_generation(default_add_generation_option());

            let (second_generation, _, _) =
                metadata.add_generation(default_add_generation_option());
            ...
            let (third_generation, _, _) = metadata.add_generation(default_add_generation_option());
