...
use crate::models::pkgdb::UpdateResult;
...
impl CoreEnvironment<ReadOnly> {
    ...
    pub fn update(
        ...
        let result: UpdateResult =
            serde_json::from_value(call_pkgdb(pkgdb_cmd).map_err(EnvironmentError2::UpdateFailed)?)
    ...
    /// so that calling `pkgdb lock` with the new lockfile_contents is
    /// idempotent.
    ///
...
impl CoreEnvironment<ReadWrite> {
    ...
    fn update_lockfile(&mut self, contents: &impl AsRef<str>) -> Result<(), EnvironmentError2> {
