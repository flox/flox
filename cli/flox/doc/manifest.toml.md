...
`floxhub-env`
: The floxhub-env name of an environment to include. This has the same semantics as
  the `--reference <env-ref> --upstream` flag passed to many Flox commands.
  That is the Flox will include the referenced environment as it is stored
  on FloxHub.
