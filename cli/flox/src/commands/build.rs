...
impl Build {
    ...
    async fn build(
        ...
        // Only expressions read this, and a manifest build's `${pkg}`
        // references can reach any of the project's — so `lock_rel_paths`,
        // which covers exactly those, decides whether it is worth a round trip.
            ...
            (&expression_nixpkgs_url, &base_nixpkgs_url)
        {
            ...
            // A manifest build that consumes an expression build's output
            // links across two package sets, and nothing downstream detects it.
            let diverged = if has_manifest_build {
                diverged_from(&locked_nixpkgs_urls(&lockfile), expression_nixpkgs)
            } else {
                Vec::new()
            };

            // Resolved fresh per invocation and moving with the catalog, so a
            // build that breaks against an unchanged tree has nothing else to
            // point at. The warning names it too, so say it once.
...
/// The revisions among `locked` that differ from `expression`, each named once.
///
/// Compared by revision, not by url: the same revision reached through a
/// differently shaped url is the same package set. A url with no revision has
/// only itself to compare.
fn diverged_from(locked: &[BaseCatalogUrl], expression: &BaseCatalogUrl) -> Vec<String> {
...
/// A nixpkgs revision short enough to compare by eye against another, which is
/// the only reason it is printed. Falls back to the url when there is no
/// revision to name.
...
///
/// `selects_nothing` comes from the caller because reachable is wider than
/// named: a manifest build's `${pkg}` reference pulls in the project's
/// expressions, and the selection applies to those.
...
///
/// The environment's own locked nixpkgs is never consulted: an expression
/// build is independent of the manifest it sits next to, so the manifest
/// cannot pin, or stale, the package set the build resolves against.
pub(crate) async fn base_nixpkgs_url_from_url_select(
    ...
    match base_catalog_url_for_stability_arg(stability.as_deref(), base_catalog_info_fut).await {
        Ok(url) => Ok(url),
        // The catalog answered, so its own message — which lists the
        // stabilities that do exist — is the useful one.
...
///
/// Reaching the catalog server is unconditional for a Nix expression build —
/// nothing in the project records the revision — so this failure needs to name
/// the way past it. The cause is folded into the message rather than left as a
/// source: `main` renders an uncategorized error by joining its whole chain
/// with ": ", which would otherwise append the transport error to the end of
/// the advice sentence and bury the one actionable line.
...
mod test {
    ...
    fn divergence_is_decided_and_deduped_by_revision() {
        ...
        assert_eq!(
            diverged_from(&[same_rev_other_shape], &expression),
            Vec::<String>::new()
        );
        ...
        assert_eq!(diverged_from(&[other, other_again], &expression), vec![
            "rev def5678".to_string()
        ]);
    }
