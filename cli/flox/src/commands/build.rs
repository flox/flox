...
impl Build {
    ...
    async fn clean(flox: Flox, mut env: ConcreteEnvironment, packages: Vec<String>) -> Result<()> {
        ...
        let expression_ref = NixFlakeref::from_path(env.dot_flox_path()).unwrap(); // TODO: decouple from env
...
pub(crate) fn check_git_tracking_for_expression_builds<'p>(
    ...
    let rel_project_path = expression_parent_dir.strip_prefix(git.path()).unwrap();

    let expression_git_ref = NixFlakeref::from_git_with_dir(
        &Url::from_directory_path(git.path()).unwrap(),
