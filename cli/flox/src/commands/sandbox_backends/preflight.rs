...
/// display name, the quoted backend id, and the upgrade instructions differ.
...
///
/// Runs `<path> --version`, parses the output with [`parse_cli_version`], and
/// bails when the version is below `check.min_version`. A failed or
/// unparseable `--version` invocation skips the gate (logged at debug) rather
/// than blocking on an unknown output format — the same tolerate-unparseable
/// semantics each backend relied on before this was shared.
    ...
    let raw = match output {
        ...
        _ => {
            debug!(
                ...
                "could not run '<cli> --version'; skipping version gate"
            );
            return Ok(());
        },
    };
