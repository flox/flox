...
impl Pull {
    ...
    fn pull_remote_environment_updates(
        ...
        // Check if the remote environment is already cached locally
        // If not, [RemoteEnvironment::new] will pull it already
        // and we don't need to do so again.
        let was_cached_before = !RemoteEnvironment::is_cached(flox, &pointer);

        // Open the remote environment and pull updates
        let mut remote_env = RemoteEnvironment::new(flox, pointer.clone(), None)?;
        let pull_result = if was_cached_before {
            remote_env.pull(flox, force)?
        } else {
            // No need to pull again after opening a remote env for the first time.
            PullResult::Updated
        };
