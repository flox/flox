...
impl Activate {
    pub async fn handle(self, mut config: Config, mut flox: Flox) -> Result<()> {
        ...
        let canonical_env_detail = env_detail_from_concrete(&concrete_environment);
        let canonical_mode = options
            .mode
            .clone()
            .unwrap_or(ActivateMode::Dev)
            .to_string();
        if let Err(err) =
            EventsHub::global().record_environment_activate_with(canonical_env_detail, |p| {
                p.with_start_services(options.start_services)
                    .with_mode(canonical_mode)
            })
        {
...
impl ActivateOptions {
    ...
    pub async fn activate(
        ...
        if is_ephemeral {
            ...
            // Record + flush the canonical event synchronously here so the
            ...
            if let Err(err) = hub.record_command_completed("activate".to_string()) {
                debug!(
                    error = %err,
                    "Failed to record canonical cli.command_completed event before exec"
                );
            }
            if let Err(err) = hub.flush(true) {
                debug!(
                    error = %err,
                    "Failed to flush canonical events before exec"
                );
            }
