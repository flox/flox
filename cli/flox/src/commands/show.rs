...
fn render_show_catalog(
    ...
    // Print Source URL
    if !first_pkg.locked_url.is_empty() {
        writeln!(writer, "Source:  {}", first_pkg.locked_url)?;
    }
    ...
    writeln!(
        writer,
        "Date Published: {}",
        first_pkg.rev_date.format("%Y-%m-%d %H:%M:%S UTC")
    )?;

    writeln!(writer)?; // Empty line before versions list
    writeln!(writer, "Other versions `flox show {pkg_path}@<version>`:")?;
...
mod test {
    ...
    fn test_column_alignment_for_system_restrictions() {
        ...
        assert_eq!(output, indoc! {"
                ...
                Source:  https://github.com/flox/nixpkgs?rev=abc123
                Date Published: 2025-05-31 12:05:15 UTC

                Other versions `flox show pkg@<version>`:
