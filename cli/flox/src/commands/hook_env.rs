            ...
            match prompt_for_wrap_session(path, plugin, self.shell)? {
                ...
                WrapSessionConsent::NoTerminal | WrapSessionConsent::UnsupportedShell => {
                    ...
                    message::info(formatdoc! {"
                        ℹ️  Run 'flox activate --dir {dir}' to enter this environment (activation is handled by plugin '{plugin}').",
                        dir = path.display(),
                    });
                },
            }
...
/// [`prompt_for_wrap_session`]'s `UnsupportedShell` result and emit a notice
/// instead.
