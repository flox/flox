...
pub struct Edit {
    ...
    #[bpaf(external(edit_action), fallback(EditAction::EditManifest{file: None, apply: false, message: None}))]
    action: EditAction,
}
#[derive(Bpaf, Clone)]
pub enum EditAction {
                ...
                if let ConcreteEnvironment::Managed(ref managed) = detected_environment {
                    if apply {
                        managed
                            .apply_local(
                                &flox,
                                message.as_deref().unwrap_or("applying local edits"),
                            )
                            .unwrap();
                        return Ok(());
                    }
