...
fn main() -> ExitCode {
    ...
    // Resolve the canonical invocation_id once per process so the early-exit
    ...
    let _canonical_events_guard = install_events_client_for_main(&config, invocation_id);
