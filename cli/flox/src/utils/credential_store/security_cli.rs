...
/// `security` returns `errSecItemNotFound` (-25300) for missing reads and
/// deletes. Its low eight bits become exit status 44, independent of locale.
...
impl SecurityCli {
    ...
    fn run(
        ...
        if missing_is_ok && output.status.code() == Some(NOT_FOUND_EXIT_CODE) {
            return Ok(None);
        }
...
mod macos_tests {
    ...
    use super::{NOT_FOUND_EXIT_CODE, SECURITY_PROGRAM, SecurityCli};
    ...
    fn missing_reads_and_deletes_return_the_expected_exit_status() {
        let keychain = TestKeychain::new();
        for subcommand in ["find-generic-password", "delete-generic-password"] {
            let mut command = Command::new(SECURITY_PROGRAM);
            command.args([
                subcommand,
                "-s",
                &keychain.cli.service,
                "-a",
                &keychain.cli.account,
            ]);
            if subcommand == "find-generic-password" {
                command.arg("-w");
            }
            let output = command
                .arg(keychain.cli.keychain.as_ref().unwrap())
                .stdin(Stdio::null())
                .output()
                .unwrap();
    ...
    fn reads_and_replaces_a_legacy_raw_token() {
        let keychain = TestKeychain::new();
        // Covers raw-token format migration; ownership transfer from a
        // differently signed flox build requires a separate manual check.
...
"#,
        ...
        fn fail(&self, status: i32, stderr: &str) {
            std::fs::write(self.dir.path().join("stderr"), stderr).unwrap();
            std::fs::write(self.dir.path().join("status"), status.to_string()).unwrap();
        }
    ...
    fn get_reports_the_failed_subcommand_and_stderr() {
        ...
        fake.fail(1, "synthetic read failure\n");
    ...
    fn set_attempts_write_after_delete_failure_and_reports_write_failure() {
        let fake = FakeSecurity::new();
        fake.fail(1, "synthetic process failure\n");
    ...
    fn remove_reports_the_failed_subcommand_and_stderr() {
        let fake = FakeSecurity::new();
        fake.fail(1, "synthetic delete failure\n");
    ...
    #[test]
    fn missing_item_status_is_only_success_for_reads_and_deletes() {
        let fake = FakeSecurity::new();
        // The real macOS test verifies the tool's exit status; this tests
        // our mapping without depending on any particular stderr text.
        for stderr in ["", "échec simulé\n"] {
            fake.fail(44, stderr);

            assert_eq!(fake.cli().get().unwrap(), None);
            fake.cli().remove().unwrap();
            let error = fake.cli().set("s3cret").unwrap_err();
            assert!(
                error.to_string().contains("add-generic-password"),
                "{error}"
    ...
    #[test]
    fn missing_item_words_do_not_mask_other_exit_statuses() {
        let fake = FakeSecurity::new();
        fake.fail(1, "synthetic failure: could not be found\n");

        assert!(fake.cli().get().is_err());
        assert!(fake.cli().remove().is_err());
    }
