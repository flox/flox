...
mod tests {
    ...
    fn logout_does_not_cache_absence_when_the_source_probe_fails() {
        temp_env::with_var(FLOXHUB_TOKEN_ENV_VAR, None::<&str>, || {
            for persistent_read_error in [false, true] {
                let cache_dir = TempDir::new().unwrap();
                let keyring = MockStore::new();
                keyring.set(TOKEN).unwrap();
                // The confirmation read either finds the token after a one-shot
                // probe failure or fails again. Neither establishes absence.
                if persistent_read_error {
                    keyring.set_get_error("keyring read failed");
                } else {
