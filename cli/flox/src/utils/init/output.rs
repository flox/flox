...
fn create_registry_with_console_writer(
    ...
    // Unit-test capture events are transport, not terminal diagnostics.
    #[cfg(test)]
    let log_layer = log_layer.with_filter(tracing_subscriber::filter::filter_fn(|metadata| {
        metadata.target() != message::test_helpers::CAPTURE_TARGET
    }));
        ...
        temp_env::with_var("FLOX_LOG", Some("off"), || {
            ...
            let events = with_captured_events(|| {
                ...
                tracing::subscriber::with_default(subscriber.with(capture), || {
                    ...
                    message::plain("presentation only");
                    let error = anyhow::anyhow!("request failed").context("installation failed");
