...
def test_tier1_reviewers_get_weight_three():
    for login in TIER1:
...
def test_tier2_reviewers_get_weight_two():
    for login in TIER2:
        r = classify(login, "User")
        assert r.tier == 2
        assert r.weight == 2.0
...
def test_unknown_human_gets_weight_one():
    r = classify("random_contributor", "User")
    assert r.tier == 3
    assert r.weight == 1.0


def test_bot_gets_weight_zero_regardless_of_login():
    r = classify("ysndr", "Bot")
    assert r.tier == 4
    assert r.weight == 0.0


def test_seed_rows_covers_both_tiers():
    rows = seed_rows()
