...
function floxUpgrade() {
	...
	for pkg in "${upgradeArgs[@]}"; do
		...
		# TODO: don't parse URIs with `jq'.
		case "$pkgArg" in
			# Upgrade by installable URI: `nixpkgs#cowsay'
			*#*)
				if ! position="$(
						manifest "$wdman" flakerefToPosition "$pkgArg"
					 )"
			...
			;;
			# Upgrade by index: 0, 420, etc
			+([0-9]))
				position="$pkgArg";
			;;
			# Fallback to upgrading by Nix store path ( hopefully )
            # Honestly I don't think this one makes any sense though because
			# of the store path's hashes.
		    *)
				if ! position="$(
					 	manifest "$wdman" storepathToPosition "$pkgArg"
					 )"
...
function floxDestroy() {
	...
	if [ $force -gt 0 ] || boolPrompt "Are you sure?" "no"; then
		...
		if [ -n "$origin" ]; then
			...
			floxmetaHelperGit -C "$environmentMetaDir" push origin --delete "$branchName" || true
		fi
...
function floxPushPull() {
	...
	# Then push or pull.
	if [ "$action" = "push" ]; then
		floxmetaHelperGit -C $tmpDir push $forceArg upstream origin/"$branchName":refs/heads/"$branchName" ||
		...
		floxmetaHelperGit -C "$environmentMetaDir" fetch --quiet origin
...
function floxGit() {
	...
	# set $branchName,$floxNixDir,$environment{Name,Alias,Owner,System,BaseDir,BinDir,ParentDir,MetaDir}
	eval $(decodeEnvironment "$environment")
	floxmetaHelperGit -C $environmentMetaDir ${args[@]}
}
