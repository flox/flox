...
start_services_blocking() {
  ...
  if [ -n "${_FLOX_SERVICES_TO_START:-}" ]; then
    ...
    COMPOSE_SHELL="$_bash" "$_strace" "$_setsid" "$_setsid" "$_process_compose" up "${services_to_start[@]}" -f "$config_file" -u "$socket_file" -L "$log_file" --tui=false > /dev/null 2>&1 &
  else
    COMPOSE_SHELL="$_bash" "$_strace" "$_setsid" "$_setsid" "$_process_compose" up -f "$config_file" -u "$socket_file" -L "$log_file" --tui=false > /dev/null 2>&1 &
  fi
