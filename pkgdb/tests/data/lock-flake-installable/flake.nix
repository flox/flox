    ...
    in
    {
      packages = eachDefaultSystemMap (
        ...
        in
        {
          ...
          licenseString = pkgs.runCommand "licenseString" { meta.license = "Unlicense"; } ''
            touch $out
          '';

          licenseAttrs = pkgs.runCommand "licenseAttrs" { meta.license = pkgs.lib.licenses.unlicense; } ''
            touch $out
          '';
          ...
          names = pkgs.runCommand "explicit-name" { pname = "hello"; } ''
            touch $out
          '';

          versioned = pkgs.runCommand "explicit-name" { version = "1.0"; } ''
            touch $out
          '';

          # with broken = true, the package does not even evaluate
          broken = pkgs.runCommand "broken" { meta.broken = false; } ''
            exit 1
          '';

          # with unfree = true, the package does not even evaluate
          unfree = pkgs.runCommand "unfree" { meta.unfree = false; } ''
            touch $out
          '';

          priority = pkgs.runCommand "priority" { meta.priority = 10; } ''
            touch $out
          '';
        }
      );
