...
@test "parse descriptor 'hello'" {
  run "$PKGDB_BIN" parse descriptor --to manifest "hello"
  assert_success
  assert_equal $(echo "$output" | jq -r '.name') "hello"
  unset output
  run "$PKGDB_BIN" parse descriptor --to query "hello"
  assert_success
...
@test "parse descriptor 'nixpkgs:hello'" {
  run "$PKGDB_BIN" parse descriptor --to manifest "nixpkgs:hello"
  assert_success
  assert_equal $(echo "$output" | jq -r '.name') "hello"
  assert_equal $(echo "$output" | jq -r '.input.id') "nixpkgs"
  unset output
  run "$PKGDB_BIN" parse descriptor --to query "nixpkgs:hello"
  assert_success
  assert_equal $(echo "$output" | jq -r '.pnameOrAttrName') "hello"
}

@test "parse descriptor 'nixpkgs:hello@1.2.3'" {
  run "$PKGDB_BIN" parse descriptor --to manifest "nixpkgs:hello@1.2.3"
  assert_success
  assert_equal $(echo "$output" | jq -r '.name') "hello"
  assert_equal $(echo "$output" | jq -r '.input.id') "nixpkgs"
  assert_equal $(echo "$output" | jq -r '.version') "1.2.3"
  unset output
  run "$PKGDB_BIN" parse descriptor --to query "nixpkgs:hello@1.2.3"
  assert_success
  assert_equal $(echo "$output" | jq -r '.pnameOrAttrName') "hello"
  assert_equal $(echo "$output" | jq -r '.version') "1.2.3"
}
...
@test "parse descriptor 'nixpkgs:hello@=1.2'" {
  run "$PKGDB_BIN" parse descriptor --to manifest "nixpkgs:hello@=1.2"
  assert_success
  assert_equal $(echo "$output" | jq -r '.name') "hello"
  assert_equal $(echo "$output" | jq -r '.input.id') "nixpkgs"
  assert_equal $(echo "$output" | jq -r '.version') "1.2"
  unset output
  run "$PKGDB_BIN" parse descriptor --to query "nixpkgs:hello@=1.2"
  assert_success
  assert_equal $(echo "$output" | jq -r '.pnameOrAttrName') "hello"
  assert_equal $(echo "$output" | jq -r '.version') "1.2"
}
...
@test "parse descriptor 'nixpkgs:packageset.hello@1.2.3'" {
  ...
  assert_equal $(echo "$output" | jq -r '.version') "1.2.3"
  unset output
  run "$PKGDB_BIN" parse descriptor --to query "nixpkgs:packageset.hello@1.2.3"
  assert_success
  assert_equal $(echo "$output" | jq -r -c '.relPath') '["packageset","hello"]'
  assert_equal $(echo "$output" | jq -r '.version') "1.2.3"
}
