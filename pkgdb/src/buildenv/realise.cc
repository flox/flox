...
#ifndef ACTIVATION_SCRIPT
#  error "ACTIVATION_SCRIPT must be set to the activation script"
#endif
...
addActivationScript( const std::filesystem::path & tempDir )
{
  ...
  debugLog( nix::fmt( "linking script to scripts dir: src=%s, dest=%s",
                      ACTIVATION_SCRIPT,
                      scriptPath ) );
  std::filesystem::create_symlink( ACTIVATION_SCRIPT, scriptPath );
}
...
makeActivationScripts( nix::EvalState & state, resolver::Lockfile & lockfile )
{
  ...
  references.insert( state.store->parseStorePath( ACTIVATION_SCRIPT ) );
